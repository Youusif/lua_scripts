...
local hitbox_enabled = ui_new_checkbox("VISUALS", "Player ESP", "Hitbox")
local hitbox_color = ui.new_color_picker("VISUALS", "Player ESP", "Hitbox color", 255, 255, 255)

local function on_player_death(e)
  if not ui_get(hitbox_enabled) then
    return
  end

  local color_r, color_g, color_b = ui_get(hitbox_color)
  ...
  if attacker_entindex == local_entindex then
    client_draw_hitboxes(victim_entindex, 2.2, 19, color_r, color_g, color_b)
  end
end
...
local function on_player_hurt(e)
  ...
  local userid, attacker_userid = e.userid, e.attacker
  if userid == nil then
    return
  end
  ...
  if entity_is_enemy(entindex) and entindex ~= 0 and attacker_entindex == local_entindex then
    if entindex ~= local_entity then
      client_draw_hitboxes(victim_entindex, 0.5, 19, color_r, color_g, color_b)
    end
  end
end
